...
const EventDetailRegistered = props => {
  ...
  const cta = (
    <a href={directionsLink} target="_new" className="btn btn-primary" style={{marginTop: 0}}>
      {fields.ctaText.value}
    </a>
  );

  const icon = <button onClick={promptToReceiveNotifications} className="notification-icon" />;
